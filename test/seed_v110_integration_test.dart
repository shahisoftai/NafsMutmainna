...
void main() {
  ...
  group('Seed v1.1.0', () {
    test('all 50 quran_ayat rows present with non-empty content', () async {
    ...
    test('replacement ayat 39 is Al-Isra 17:37, not the broken 2:102', () async {
    ...
    test('replacement ayat 49 is the REAL At-Tawbah 9:112 (repentant worshippers)', () async {
      final rows = await appDb.db.query(
        'quran_ayat',
        where: 'Ayat_ID = ?',
        whereArgs: [49],
    ...
    test('full verses are present (no truncated fragments)', () async {
      ...
      final previouslyTruncated = [4, 6, 9, 10, 12, 15, 17, 19, 20, 21, 22, 24, 25, 26, 27, 28, 29, 30, 31, 33, 34, 36, 37, 40, 41, 42, 47];
      for (final aid in previouslyTruncated) {
        ...
        // typically 100+ chars).
        expect(ar.length, greaterThan(80),
            reason: 'Ayat $aid Arabic too short (${ar.length} chars) — likely truncated');
        final en = rows.first['English_Translation'] as String;
        expect(en.length, greaterThan(80),
            reason: 'Ayat $aid English too short (${en.length} chars) — likely truncated');
      }
    });

    test('emotion_quran_links reflects the 4 removals and 1 addition', () async {
      final links = await appDb.db.query('emotion_quran_links');

      // Removed
      expect(
        links.where((l) => l['Ayat_ID'] == 12 && l['Emotion_ID'] == 37).isEmpty,
        isTrue,
        reason: '8:2 → Love for People should be removed',
      );
      expect(
        links.where((l) => l['Ayat_ID'] == 12 && l['Emotion_ID'] == 44).isEmpty,
        isTrue,
        reason: '8:2 → Certainty should be removed',
      );
      expect(
        links.where((l) => l['Ayat_ID'] == 37 && l['Emotion_ID'] == 18).isEmpty,
        isTrue,
        reason: '16:116 → Suspicion should be removed',
      );
      expect(
        links.where((l) => l['Ayat_ID'] == 45 && l['Emotion_ID'] == 35).isEmpty,
        isTrue,
      ...
      // Added
      expect(
        links.where((l) => l['Ayat_ID'] == 39 && l['Emotion_ID'] == 11).isNotEmpty,
        isTrue,
  ...
  group('Recommend v2 with real seed', () {
    ...
    test('Pride (emotion 12) at intensity 8 gets the top-weighted Quran card', () async {
